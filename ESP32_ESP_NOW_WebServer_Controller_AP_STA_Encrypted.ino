/*Russ Sveda 2023: ESP32 ESP-NOW Web Server Controller AP STA Mode Encrypted - This example controls the on-board LED of three reciever boards.  This can be adapted to control other GPIOs to operate relays, etc.
  Credits:
 ...
 */
// Arduino-IDE 2.0.4 used with a board selection of DOIT ESP32 DEVKIT V1 and actual board is NodeMCU ESP-32S V1.1
...
// Uses a User Interface Web page, store it in the same folder as the .ino file
#include "PageIndexS.h" 

// Defines the Digital Pin of the Controller On-Board LED.
...
// Define the PMK & LMK encryption keys. Must match entry in the Rcvrs to work. Change the LMK to not match the Rcvrs to confirm encryption is working 
...
void setup() {
  ...
  digitalWrite(ON_Board_LED, LOW); //--> Turn off the On Board LED
  ...
  // Set the encryption key to use with the peers
 ...
 if (esp_now_add_peer(&peerInfo) != ESP_OK){
  ...
  // If you wish connect to ESP32 controller directly then use the Soft APIP Address or via WiFi use the WiFi network IP Address.
}
